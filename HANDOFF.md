...
| **54** | Piloto técnico Woodendot sin escritura                                                                                 | adaptador UCP + seed + `docs/merchant-sourcing-round-3.md`           | 12/12 productos disponibles en EUR con destinos españoles e imágenes de origen verificadas. El runner respeta el máximo UCP de 10 IDs con lotes 10+2 y diagnóstico de deriva. Store oculta, no verificada y eco-score 0; no hay filas Cloud hasta superar Skimlinks e imagen live.            |
...
| `pnpm test` / `pnpm exec tsc --noEmit` / `pnpm build` | 74/74 · rc=0 · rc=0                                                                                                              |
...
| **B-7A** | 🟡 **Segundos merchants en los otros nichos**    | verificación Skimlinks + imagen live + activación | Woodendot: adaptador y selección de 12 pasan dry-run, sin escritura Cloud; desplegar allowlist, probar imagen y pedir gate antes de `--write`. Después: Thinking MU → Native Union (`docs/merchant-sourcing-round-3.md`). |
...
- [ ] Woodendot: comprobar `woodendot.com` y deep linking en Skimlinks; el piloto de 12 está validado pero seguirá fuera de Supabase hasta superar este gate y la prueba live de imagen.
