...
async function fetchStore(slug: string): Promise<StoreDetails | null> {
  ...
  const storeRes = await sb
    .from("stores")
    .select(
      "id, slug, name, country, eco_score, values, short_description, long_description, niche, verified, featured",
    )
    .eq("slug", slug)
    .eq("active", true)
    .maybeSingle();
...
export async function generateMetadata({
  ...
  const description =
    s.short_description ?? `Descubre los productos de ${s.name} en Shopifind.`;
