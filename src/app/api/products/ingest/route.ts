...
export async function POST(request: NextRequest) {
  ...
  const storeRes = await sb.from('stores').select('id').eq('slug', parsed.data.store_slug).single();
  const store = storeRes.data as { id: string } | null;
  if (!store?.id) return NextResponse.json({ error: 'store_not_found' }, { status: 404 });
